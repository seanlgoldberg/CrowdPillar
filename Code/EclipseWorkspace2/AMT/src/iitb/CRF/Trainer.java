...
public class Trainer {
	...
	void doTrain() {
		...
		do {
			...
			f = -1 * f; // since the routine below minimizes and we want to
						// maximize logli
			for (int j = 0; j < lambda.length; j++) {
				gradLogli[j] *= -1;
			}
			...
			if ((evaluator != null) && (evaluator.evaluate() == false))
				break;
			try {
				LBFGS.lbfgs(numF, params.mForHessian, lambda, f, gradLogli,
						false, diag, iprint, params.epsForConvergence, xtol,
						iflag);
		...
		try {
			...
			for (numRecord = 0; diter.hasNext(); numRecord++) {
				...
				if (params.debugLvl > 1) {
					Util.printDbg("Read next seq: " + numRecord + " logli "
							+ logli);
				}
				alpha_Y.assign(1);
				for (int f = 0; f < lambda.length; f++)
				...
				beta_Y[dataSeq.length() - 1].assign(1.0 / scale[dataSeq
						.length() - 1]);
				...
				for (int i = dataSeq.length() - 1; i > 0; i--) {
					...
					initMDone = computeLogMi(featureGenerator, lambda, dataSeq,
							i, Mi_YY, Ri_Y, true, reuseM, initMDone);
					tmp_Y.assign(beta_Y[i]);
					tmp_Y.assign(Ri_Y, multFunc);
					RobustMath.Mult(Mi_YY, tmp_Y, beta_Y[i - 1], 1, 0, false,
							edgeGen);
				...
				for (int i = 0; i < dataSeq.length(); i++) {
					...
					if (i > 0) {
						...
						RobustMath.Mult(Mi_YY, tmp_Y, newAlpha_Y, 1, 0, true,
								edgeGen);
					...
					while (featureGenerator.hasNext()) {
						...
						if (yprev < 0) {
							ExpF[f] += newAlpha_Y.get(yp) * val
									* beta_Y[i].get(yp);
						} else {
							ExpF[f] += alpha_Y.get(yprev) * Ri_Y.get(yp)
									* Mi_YY.get(yprev, yp) * val
									* beta_Y[i].get(yp);
						}
					}
			...
			if (params.debugLvl > 2) {
				...
				for (int f = 0; f < lambda.length; f++)
					System.out.println(featureGenerator.featureName(f) + " "
							+ grad[f] + " ");
				System.out.println(" :g");
			}

			if (params.debugLvl > 0)
				Util.printDbg("Iter " + icall + " log likelihood " + logli
						+ " norm(grad logli) " + norm(grad) + " norm(x) "
						+ norm(lambda));
			if (icall == 0) {
	...
	//	System.out.println("----\nAlpha: "+fwdVector.toString()+ "\nBeta: "+bckVector.toString());
		return logli;
	...
	//	System.out.println("MI previous: \n"+Mi_YY);
		while (featureGen.hasNext()) {
			...
			if (yprev < 0) {
				...
				// System.out.println("LAMBDA: "+oldVal+lambda[f]*val);
				Ri_Y.setQuick(yp, oldVal + lambda[f] * val);
			} else if (Mi_YY != null) {
				
				Mi_YY.setQuick(yprev, yp, Mi_YY.getQuick(yprev, yp) + lambda[f]
						* val);
	...
	static boolean computeLogMi(FeatureGenerator featureGen, double lambda[],
			DoubleMatrix2D Mi_YY, DoubleMatrix1D Ri_Y, boolean takeExp,
			boolean reuseM, boolean initMDone, int i) {
	...
	static void computeLogMi(FeatureGenerator featureGen, double lambda[],
			DataSequence dataSeq, int i, DoubleMatrix2D Mi_YY,
			DoubleMatrix1D Ri_Y, boolean takeExp) {
		computeLogMi(featureGen, lambda, dataSeq, i, Mi_YY, Ri_Y, takeExp,
				false, false);
	}

	static boolean computeLogMi(FeatureGenerator featureGen, double lambda[],
			DataSequence dataSeq, int i, DoubleMatrix2D Mi_YY,
			DoubleMatrix1D Ri_Y, boolean takeExp, boolean reuseM,
			boolean initMDone) {
		featureGen.startScanFeaturesAt(dataSeq, i);
		return computeLogMi(featureGen, lambda, Mi_YY, Ri_Y, takeExp, reuseM,
				initMDone);
	}
	...
	protected double computeFunctionGradientLL(double lambda[], double grad[]) {
		...
		try {
			...
			for (int numRecord = 0; diter.hasNext(); numRecord++) {
				...
				for (int i = dataSeq.length() - 1; i > 0; i--) {
					if (params.debugLvl > 2) {
						 ...
						 * Util.printDbg("Features fired");
						 * featureGenerator.startScanFeaturesAt(dataSeq, i);
						 * while (featureGenerator.hasNext()) { Feature feature
					...
					// compute the Mi matrix
					initMDone = computeLogMi(featureGenerator, lambda, dataSeq,
							i, Mi_YY, Ri_Y, false, reuseM, initMDone);
					tmp_Y.assign(beta_Y[i]);
					tmp_Y.assign(Ri_Y, sumFunc);
					RobustMath.logMult(Mi_YY, tmp_Y, beta_Y[i - 1], 1, 0,
							false, edgeGen);
				}
				...
				for (int i = 0; i < dataSeq.length(); i++) {
					...
					if (i > 0) {
						tmp_Y.assign(alpha_Y);
						RobustMath.logMult(Mi_YY, tmp_Y, newAlpha_Y, 1, 0,
								true, edgeGen);
					...
					while (featureGenerator.hasNext()) {
						...
						if ((dataSeq.y(i) == yp)
							...
							thisSeqLogli += val * lambda[f];
							if (params.debugLvl > 2) {
								System.out.println("Feature fired " + f + " "
										+ feature);
							}
						}
						...
						if (yprev < 0) {
							ExpF[f] = RobustMath.logSumExp(ExpF[f],
									newAlpha_Y.get(yp) + RobustMath.log(val)
											+ beta_Y[i].get(yp));
						} else {
							ExpF[f] = RobustMath.logSumExp(
									ExpF[f],
									alpha_Y.get(yprev) + Ri_Y.get(yp)
											+ Mi_YY.get(yprev, yp)
											+ RobustMath.log(val)
											+ beta_Y[i].get(yp));
						}
			...
			if (params.debugLvl > 0)
				Util.printDbg("Iteration " + icall + " log-likelihood " + logli
						+ " norm(grad logli) " + norm(grad) + " norm(x) "
						+ norm(lambda));
